...
public:
	...
	// 获取错误标题
	const char* title() const noexcept { return (const char*)error_title.c_str(); }

	// 获取错误信息
	const char* what() const noexcept { return (const char*)error_info.c_str(); }

private:
	std::u8string error_title;	// 错误标题
	std::u8string error_info;	// 错误信息
};
...
public:
	...
	{
		auto past_time = clock_t::now() - m_instance.last_time;
		m_instance.delta_time = static_cast<double>(std::chrono::duration_cast<ms_t>(past_time).count());
	...
	{
		auto cur_time = clock_t::now();		// 获取当前时间
	...
	static float get_global_time()
	{
		...
		auto global_time = std::chrono::duration_cast<ms_t>(cur_time - m_instance.global_start_time).count();
		return static_cast<float>(global_time) / 1000.0f;	// 转换为秒
	}
	...
	{
		double normal_delta = m_instance.delta_time / 1000.0;	// 转换为秒
		return static_cast<float>(normal_delta * m_instance.time_scale);
	}
